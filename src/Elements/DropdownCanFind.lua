...
function Element:New(Idx, Config)
	...
	local Dropdown = {
		...
		-- [FIX #4] Khởi tạo Value đúng cho Multi mode: phải là table, không được nil
		Value = Config.Multi and (type(Config.Default) == "table" and Config.Default or {}) or Config.Default,
		Multi = Config.Multi,
		Buttons = {},
		Opened = false,
		Type = "Dropdown",
	...
	local SearchInputRef = New("TextBox", {
	...
	-- [FIX #1] Toggle thay vì luôn Open
	...
	-- [FIX #2] Check thêm: nếu dropdown chưa mở thì bỏ qua (tránh race condition)
	-- [FIX #3] Check nếu click nằm trong DropdownInner thì không Close (tránh race condition với toggle)
	Creator.AddSignal(UserInputService.InputBegan, function(Input)
		if
			Input.UserInputType == Enum.UserInputType.MouseButton1
			or Input.UserInputType == Enum.UserInputType.Touch
		then
			-- [FIX #2] Nếu dropdown chưa mở thì không cần kiểm tra close
			if not Dropdown.Opened then
				return
			end

			-- [FIX #3] Nếu click nằm trên DropdownInner thì bỏ qua
			-- (tránh race: click mở → InputBegan fires cùng lúc → Close ngay)
			local InnerPos = DropdownInner.AbsolutePosition
			local InnerSize = DropdownInner.AbsoluteSize
			if
				Mouse.X >= InnerPos.X
				and Mouse.X <= InnerPos.X + InnerSize.X
				and Mouse.Y >= InnerPos.Y
				and Mouse.Y <= InnerPos.Y + InnerSize.Y
			then
				return
			end
	...
	-- [FIX #7] Tween Close thay vì set instant
	...
	-- [FIX #5] Dùng GetPropertyChangedSignal("Text") thay vì Changed
	-- Changed fires cho MỌI property (CursorPosition, SelectionStart...) → lag
	SearchInputRef:GetPropertyChangedSignal("Text"):Connect(function()
		if Dropdown.Opened then
			Dropdown:BuildDropdownList()
		end
	end)
	...
	function Dropdown:BuildDropdownList()
		local Values = Dropdown.Values
		local Buttons = {}
		local SearchText = string.lower(SearchInputRef.Text)
		...
		for Idx, Value in next, Values do
			...
			if not string.find(string.lower(tostring(Value)), SearchText, 1, true) then
				continue
			end
		...
		-- [FIX #5] Cập nhật Dropdown.Buttons để code bên ngoài có thể truy cập
